...
`POST /api/shares/id/{share_id}/rotate` 轮换分享凭证：请求体仅含非空字符串 `rotation_id`。原始 token 为第 1 代，每次成功轮换在最高代次上加一（首次轮换返回 `token_version=2`），原子生成新 token 并返回 201 及 `share_id`、`rotation_id`、`token`、`token_version`、UTC 微秒级 `rotated_at`；范围、期限与 `share_id` 不变。**新 token 仅在此 201 响应中出现一次**，数据库只保存各代 token 的 SHA-256 摘要。此后仅新 token 可访问，任一代旧 token 访问返回 410（未知 token 仍为 404）；撤销旧 token 仍返回 204 且不影响当前分享，当前 token 与 `share_id` 的撤销语义不变。未知 `share_id` 返回 404；分享已撤销或按当前 UTC 已过期时，未成功过的 `rotation_id` 返回 409 且状态不变。同一分享的 `rotation_id` 幂等：成功后的重试优先于过期、撤销判定，固定返回 200 及原元数据（不含 token），不改变状态；并发同一 `rotation_id` 只生成一代，且至多一代有效。代际、幂等与失效状态持久化在 SQLite（`share_rotations` 与 `share_superseded_tokens` 表）中，重启后保持。轮换不读取 `retail_members`、不扣减预算、不新增发布记录；token 及摘要不会进入列表、历史或导出。首页在分享卡片与分享管理区提供轮换按钮、新链接的一次性展示与复制，以及可读错误提示。
