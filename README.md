...
`POST /api/releases/batch` 原子批量发布：请求体含非空 `batch_id` 与 1–20 项 `requests`；每个子项沿用单条发布的字段（`request_id`、`dataset_id`、`filters`、`epsilon`）、同样的校验、Laplace 噪声与记录格式，批内 `request_id` 必须唯一。顶层或子项的结构、类型、取值错误（含未知字段）或批内 `request_id` 重复均返回 422；任一子项的 `dataset_id` 不是 `retail-demo`，整批返回 404。首次提交在**单个 SQLite 事务**内先确认全部 `request_id` 均未被单条或批量发布占用、且批内 epsilon 总和不超过剩余预算，再一次性写入整批：任一 `request_id` 冲突或预算不足返回 409，绝不部分成功，失败不留任何发布行或批次占位。成功返回 201，含 `batch_id`、按输入顺序排列的 `releases`、`total_epsilon`、最终 `remaining_budget` 与 UTC 微秒级 `created_at`；各项的 `remaining_budget` 按输入顺序逐项扣减（顶层余额即末项余额），同一 `created_at` 贯穿整批。批量记录与单条共用 `releases` 表，故历史、导出、合作方分享都原样保留逐项余额，提交后公开查询立即可见，且不泄露真实计数或行级数据。同一 `batch_id` 的规范化请求完全相同则 200 重放首次响应（同一批记录、合计与时间戳），不再采样、扣费或写入；规范化请求不同则 409（子项顺序不同视为不同请求）。幂等、原子性以及与单条共享的 `request_id` 占用和预算约束在重启、并发下均成立。

隐私预算总额默认为 3.0，可用 `PRIVSTAT_EPSILON_BUDGET` 调整。每次首发原子扣减 ε，余额不足返回 409 且不留记录；扣减与幂等记录在 SQLite 中持久化，重启与并发下均成立。`GET /api/privacy-budget` 返回初始、已用与剩余额度，`GET /api/releases` 按时间倒序返回发布历史。首页支持筛选发布、批量提交与结果展示、预算与历史刷新及可读错误提示。
...
`POST /api/shares/id/{share_id}/rotate` 轮换分享凭证：请求体**仅**接受非空字符串 `rotation_id`，出现任何额外字段返回 422。原始 token 为第 1 代，每次成功轮换在最高版本上加一（首次返回 `token_version=2`），在单事务内生成新 token 并使旧代失效，返回 201 及 `share_id`、`rotation_id`、`token`、`token_version`、UTC 微秒级 `rotated_at`；范围、期限与 `share_id` 不变。**新 token 仅在此 201 响应中出现一次**，SQLite 的 `share_tokens` 表只保存各代 token 的 SHA-256 摘要。此后仅新 token 可访问，旧 token 访问返回 410；旧 token 撤销仍返回 204 且不影响当前分享，当前 token 与 `share_id` 撤销沿用原语义。未知 `share_id` 返回 404；分享已撤销或按当前 UTC 已过期时，未成功过的 `rotation_id` 返回 409 且状态不变。同一分享的 `rotation_id` 幂等：成功后的重试优先于过期、撤销判定，固定返回 200 及原元数据（不含 token），不改变状态；并发同标识只生成一代，任何时刻至多一代有效，代际、幂等与失效状态重启后保持。轮换不读取 `retail_members`、不扣减预算、不新增发布记录，token 与摘要不会进入列表、历史或导出。
